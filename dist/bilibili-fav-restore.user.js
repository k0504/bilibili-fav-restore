 ...
 * Source: bilibili-fav-list-fix-core.js (CORE_VERSION = 0.8.5)
...
(function () {
    ...
    async function patchOnce() {
        ...
        // Paint loading indicator on every fresh hit BEFORE resolveItems
        // — even cache hits go through a microtask so the user gets a
        // momentary "yes the script noticed this card" flash, and slow
        // 3rd-party fetches no longer look like the script is dead.
        hits.forEach(markLoading);

        var neededAvs = hits.map(getAvFromHit).filter(Boolean);
        var merged;
        try { merged = await resolveItems(neededAvs, mediaId); }
        catch (e) {
            warn('resolve failed:', e);
            toast('数据解析失败：' + e.message, 'err');
            // Resolve threw — never going to call markPatched, so the
            // loading pulse would otherwise stick forever. Clear it.
            hits.forEach(clearLoading);
            return;
        }
        ...
        try {
            ...
            // Catch loading-state leaks. markPatched() clears loading on the
            // success/nodata paths; this finally catches the rest:
        ...
        if (patched > 0 || unrecoverable > 0) {
            log('patched', patched, '/ unrecoverable', unrecoverable, '/ total', hits.length);
        }
