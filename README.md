...
- **卡片三点菜单**：菜单中提供「停止重试」与「恢复重试」两项，按卡片当前状态互斥显示。标题已还原、仅封面仍在后台补取的条目不显示按钮，菜单是其唯一入口。

停止的对象是网络重试，不影响本地数据：已手动备份的条目仍会照常以本地备份还原，停止记录亦不受「清除所有缓存」影响。恢复重试将一并清除该条目的还原缓存并立即重新抓取一轮。

如需一次性清空全部停止记录，在 Tampermonkey 菜单中选择 **fav-fix：清除所有「停止重试」标记**，脚本将报告清除的手动与自动记录条数并重绘当前页面的按钮与状态标签，无需刷新。亦可在 DevTools Console 中以 `__biliFavFix.noRetry` 查询与操作该列表。
